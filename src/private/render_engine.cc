...
void RenderEngine::Render() {
    ...
    for(int i = 0; i < thread_nums; i ++) {
        ...
        th[i] = std::make_unique<std::thread>(std::bind(&RenderEngine::ThreadFunction, this, scene_, camera, std::ref(show_image), starti, endi));
    }
    for(auto& it: th) {
        it->detach();
    }
...
void RenderEngine::ThreadFunction(const SceneObjects& scene, const Camera& camera, ShowImage& show_image, int starti, int endi)
{
